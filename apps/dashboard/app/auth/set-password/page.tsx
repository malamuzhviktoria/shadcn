...
type DemoState = "valid" | "invalid-link"
      ...
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Your account is now active. A confirmation email has been sent to your
            registered address.
          </p>
        </div>
  ...
  return (
    <>
      ...
      <p className="mb-6 text-sm text-muted-foreground">
        Create a password to access the Spectrum Clean Manager Web App.
      </p>
...
function InvalidRegistrationLink() {
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/10 px-4 py-3 dark:border-destructive/40">
        <AlertCircle className="mt-0.5 size-4 shrink-0 text-destructive" />
        <div>
          <p className="text-sm font-medium text-foreground">
            Registration link is no longer valid
          </p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            This link has expired or has already been used.
            Registration links are valid for 24 hours.
          </p>
        </div>
      </div>
      <p className="text-sm text-muted-foreground">
        Contact Head Office to request a new registration link.
      </p>
      <Link
        href="/auth/login"
        className="inline-flex h-10 w-full items-center justify-center gap-1.5 rounded-md border border-input bg-muted/50 text-sm font-medium transition-colors hover:bg-muted"
      >
    ...
    <div className="flex min-h-full w-full flex-col items-center justify-center bg-muted/30 px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border border-border bg-card p-8 shadow-sm">
          {demoState === "valid" ? (
            <SetPasswordForm key="set-password-form" />
          ) : (
            <>
        ...
        <div className="mt-5 rounded-lg border border-border bg-muted/40 p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(["valid", "invalid-link"] as DemoState[]).map(s => (
              <button
                key={s}
              ...
              >
                {s === "valid" && "Valid link"}
                {s === "invalid-link" && "Invalid / expired link"}
              </button>
            ))}
          </div>
        </div>
